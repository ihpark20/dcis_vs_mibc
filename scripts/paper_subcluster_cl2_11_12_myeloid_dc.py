"""CL2 + CL11 + CL12 통합 Macrophage/pDC/cDC (단핵식세포계) 서브클러스터링.

CL2 (macrophage M1/M2/moDC 55,938) + CL11 (pDC 관련 6,324) + CL12 (cDC 2,693)을
합쳐 myeloid+DC 마커 하나라도 양성인 세포만 필터 후 재클러스터링.
subtype/마커/프로그램 + 코어 dominance + 서브클러스터별 DCIS vs MIBC 비교.

전략:
  1. CL2+CL11+CL12 subset (QC FAIL 제외)
  2. myeloid/DC 마커(>0) 하나라도 양성 필터
  3. myeloid+DC focused feature로 PCA→Harmony(slide)→Leiden
  4. 프로그램 스코어 + 코어 dominance(Top1/2/3) + DCIS vs MIBC(코어 단위 검정)

출력:
    03.data_processed/cl2_11_12_myeloid_dc_subclustered.h5ad
    04.tables/cl2_11_12_myeloid_dc_{marker_genes,program_scores,summary,dcis_vs_mibc_test}.csv
    05.figures/cl2_11_12_myeloid_dc_{umap_subclusters,umap_program_scores,dotplot_markers,
              program_score_heatmap,dcis_vs_mibc_boxplot}.png
"""
...
OUT_H5AD = PROJ / "03.data_processed/subclustered/cl2_11_12_myeloid_dc_subclustered.h5ad"
...
TARGET_CLUSTERS = ["2", "11", "12"]
RESOLUTION = 0.4
RANDOM_STATE = 42
N_PCS = 15
LABEL = "cl2_11_12_myeloid_dc"
QC_FAIL = {("TMA1", "D4"), ("TMA1", "D8")}
PATH_COLORS = {"dcis": "#4ECDC4", "mibc": "#FF6B6B"}

# myeloid + DC 마커 하나라도 양성 → 유지
FILTER_GENES = [
    "CD68",
    "CD163",
    "CD14",
...
FEATURE_GENES = [
    # core macrophage / mono-phagocyte
    "CD68",
    "CD14",
    "LYZ",
    "AIF1",
...
# subtype 라벨 결정 (계통 프로그램; core_macrophage/ifn는 modifier라 제외)
SUBTYPE_PROGS = [
    "macrophage_M1",
    "macrophage_M2",
    "monocyte",
...
def main():
    print("1. CL2+CL11+CL12 subset + filter")
    ...
    print(f"  이전 celltype: {sub.obs['celltype_final'].value_counts().head(6).to_dict()}")
    ...
    for cl in sub_cats:
        ...
        parent_pct = cells["leiden_parent"].value_counts(normalize=True) * 100
        ...
        subtype_label = max(
            SUBTYPE_PROGS, key=lambda p: score_mat.loc[cl].get(f"score_{p}", -np.inf)
        )
        ...
        rows.append(
            {
                ...
                "pct_CL2": round(parent_pct.get("CL2", 0), 1),
                "pct_CL11": round(parent_pct.get("CL11", 0), 1),
                "pct_CL12": round(parent_pct.get("CL12", 0), 1),
    ...
    print("\n10. DCIS vs MIBC (코어 단위 비율, Mann-Whitney + BH FDR)")
    ct = (
    ...
    sidx = summary_df.set_index("leiden_sub")
    test_rows = []
    for cl in sub_cats:
    ...
    axes[0].set_title(f"CL2+CL11+CL12 sub (res={RESOLUTION}, n={n_sub})")
    sc.pl.umap(
        sub,
        color="leiden_parent",
        ax=axes[1],
        show=False,
        frameon=False,
        palette={"CL2": "#1f77b4", "CL11": "#2ca02c", "CL12": "#d62728"},
    )
    axes[1].set_title("Parent (CL2 mac / CL11 pDC / CL12 cDC)")
    ...
    fig, ax = plt.subplots(figsize=(max(8, len(program_keys) * 0.8), max(4, n_sub * 0.45)))
    ...
    for i in range(n_sub):
        for j in range(len(program_keys)):
            ax.text(j, i, f"{score_mat.values[i, j]:.1f}", ha="center", va="center", fontsize=6)
    ax.set_title("CL2+CL11+CL12 × 프로그램 (z)", fontweight="bold")
    plt.tight_layout()
    ...
    w = 0.38
    fig, ax = plt.subplots(figsize=(max(12, len(sub_cats) * 0.9), 6))
    bp_d = ax.boxplot(
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f"sub{cl}\n{sidx.loc[cl, 'subtype'][:10]}" for cl in sub_cats], fontsize=7)
    ax.set_ylabel("코어 내 비율 (myeloid/DC 중)")
    ax.set_title("CL2+CL11+CL12 서브클러스터: DCIS vs MIBC (코어 단위, BH-FDR)")

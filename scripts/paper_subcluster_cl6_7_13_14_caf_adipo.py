"""CL6 + CL7 + CL13 + CL14 통합 CAF + Adipocyte 서브클러스터링.

CL6/CL7/CL14 (CAF 우세) + CL13 (실제 adipocyte, ADIPOQ+ 93%)을 합쳐
CAF+adipocyte 마커 하나라도 양성인 세포만 필터 후 재클러스터링.
지방세포 ↔ 지방섬유아 ↔ CAF 의 간질 연속 스펙트럼을 한 공간에서 본다.
subtype/마커/프로그램 + 코어 dominance + 서브클러스터별 DCIS vs MIBC 비교.

출력:
    03.data_processed/cl6_7_13_14_caf_adipo_subclustered.h5ad
    04.tables/cl6_7_13_14_caf_adipo_{marker_genes,program_scores,summary,dcis_vs_mibc_test}.csv
    05.figures/cl6_7_13_14_caf_adipo_{umap_subclusters,umap_program_scores,dotplot_markers,
...
OUT_H5AD = PROJ / "03.data_processed/subclustered/cl6_7_13_14_caf_adipo_subclustered.h5ad"
...
TARGET_CLUSTERS = ["6", "7", "13", "14"]
RESOLUTION = 0.4
RANDOM_STATE = 42
N_PCS = 15
LABEL = "cl6_7_13_14_caf_adipo"
QC_FAIL = {("TMA1", "D4"), ("TMA1", "D8")}
PATH_COLORS = {"dcis": "#4ECDC4", "mibc": "#FF6B6B"}

# CAF + adipocyte 마커 하나라도 양성 → 유지
FILTER_GENES = [
    "LUM",
    "PDGFRA",
    "PDGFRB",
...
def main():
    print("1. CL6+7+13+14 subset + filter")
    ...
    print(f"  filter genes({len(filter_idx)})")
        ...
        rows.append(
            {
                ...
                "pct_CL6": round(parent_pct.get("CL6", 0), 1),
                "pct_CL7": round(parent_pct.get("CL7", 0), 1),
                "pct_CL13": round(parent_pct.get("CL13", 0), 1),
                "pct_CL14": round(parent_pct.get("CL14", 0), 1),
    ...
    axes[0].set_title(f"CL6+7+13+14 sub (res={RESOLUTION}, n={n_sub})")
    sc.pl.umap(
        sub,
        color="leiden_parent",
        ax=axes[1],
        show=False,
        frameon=False,
        palette={"CL6": "#1f77b4", "CL7": "#2ca02c", "CL13": "#d62728", "CL14": "#9467bd"},
    )
    axes[1].set_title("Parent (CL13=adipo)")
    ...
    ax.set_title("CL6+7+13+14 × 프로그램 (z)", fontweight="bold")
    plt.tight_layout()
    ...
    ax.set_ylabel("코어 내 비율 (CAF+adipo 중)")
    ax.set_title("CL6+7+13+14 서브클러스터: DCIS vs MIBC (코어 단위, BH-FDR)")
